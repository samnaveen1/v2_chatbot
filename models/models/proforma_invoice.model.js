...
const proformaInvoiceSchema = new mongoose.Schema({
    ...
    proformaInvoiceDate: { type: Date, required: true },
    ...
    proformaInvoiceValidUpto: { type: Date, required: false },
...
proformaInvoiceSchema.pre('findOneAndUpdate', async function (next) {
    try {
...
const model = mongoose.model('proforma_invoices', proformaInvoiceSchema);
const modelSecondary = secondaryDB.model('proforma_invoices', proformaInvoiceSchema);

/// Proforma invoice request Activity
const collectionActivitySchema = new mongoose.Schema({
    proformaInvoiceId: { type: mongoose.Schema.Types.ObjectId, required: [true, "proformaInvoiceId is required"] },
...
const modelActivity = activitiesDB.model('proforma_invoice_activities', collectionActivitySchema);
const modelActivitySecondary = activitiesSecondaryDB.model('proforma_invoice_activities', collectionActivitySchema);

/**
 * Activity Schema for Validation
 */
const proformaInvoiceActivityJoiSchema = Joi.object({
    proformaInvoiceId: Joi.object().required(),
...
/// create activity
const createActivity = async function (proformaInvoiceActivityData) {
    if (proformaInvoiceActivityData.what.oldValues && proformaInvoiceActivityData.what.newValues) {

        let oldValues = JSON.parse(JSON.stringify(proformaInvoiceActivityData.what.oldValues))
        let newValues = JSON.parse(JSON.stringify(proformaInvoiceActivityData.what.newValues))

        await getDistinctValues(oldValues, newValues).then(async (result) => {
            if (result) {
                proformaInvoiceActivityData.what.oldValues = result.old && Object.keys(result.old).length > 0 ? result.old : null
                proformaInvoiceActivityData.what.newValues = result.new && Object.keys(result.new).length > 0 ? result.new : null

                // proformaInvoiceActivityData.what.oldValues === null && proformaInvoiceActivityData.what.newValues === null ? delete proformaInvoiceActivityData.what : null
            }
        })
    }
    return new Promise(function (resolve, reject) {
        Joi.validate(proformaInvoiceActivityData, proformaInvoiceActivityJoiSchema, { abortEarly: false }).then(async (proformaInvoiceActivityData) => {
            await modelActivity(proformaInvoiceActivityData).save().then((activity) => { resolve(activity); }).catch((err) => { reject(err); });
