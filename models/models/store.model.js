...
const storeSchema = new mongoose.Schema({
  ...
  storeType: { type: String, required: false, default: "Physical" },
  storeIncharge: { type: String, required: false, default: null },
  storePhone: { type: String, required: false, default: null },
  storeEmail: { type: String, required: false, default: null },

  billNumberStartFrom: { type: Number, default: 0 },
  billLastSequenceNumber: { type: Number, required: false, default: null },
  returnBillNumberStartFrom: { type: Number, default: 0 },
  returnBillLastSequenceNumber: { type: Number, required: false, default: null },
  podBillNumberStartFrom: { type: Number, default: 0 },
  podBillLastSequenceNumber: { type: Number, required: false, default: null },
  invoiceBillNumberStartFrom: { type: Number, default: 0 },
  invoiceLastSequenceNumber: { type: Number, required: false, default: null },

  inwardNumberStartFrom: { type: Number, default: 0 },
  inwardLastSequenceNumber: { type: Number, required: false, default: null },

  storeTags: { type: Array, required: false, default: null },
  mapLocation: {
  ...
  storePreferences: [{
    ...
    preferenceValue: { type: mongoose.Schema.Types.Mixed, required: false },
    preferenceCategory: { type: String, required: true },
    preferenceGroup: { type: String, required: true },
  ...
  allowLoyaltyRedeemToBills: { type: Boolean, default: true },
  allowLoyaltyCalculationToBills: { type: Boolean, default: true },
  status: { type: Boolean, default: true },
  autoCompleteOutwards: { type: Boolean, default: true },
...
const modelSecondary = secondaryDB.model('stores', storeSchema);
...
const modelActivity = activitiesDB.model('store_activities', storeActivitySchema);
const modelActivitySecondary = activitiesSecondaryDB.model('store_activities', storeActivitySchema);

/**
 * Activity Schema for Validation
 */
const storeActivityJoiSchema = Joi.object({
  storeId: Joi.object(),
...
const createActivity = async function (activityDetails) {
  ...
  return new Promise(function (resolve, reject) {
    if(activityDetails.what){
      Joi.validate(activityDetails, storeActivityJoiSchema, { abortEarly: false }).then(async (activityData) => {
  ...
  return new Promise(function (resolve, reject) {
    ...
    model.findOne(where, {}).then(async (selectedData) => {
      ...
      if (selectedData && selectedData.isDeleteLocked == false) {
          ...
          createActivity(activityLog).then((history) => {
            ...
            if (history) {
              ...
              let approvalRequest = {
                ...
                "approvalTitle": selectedData.storeName,
                "requestType": "Delete",
                "who": {
        ...
        reject({
          error: "store not allowed to delete!",
