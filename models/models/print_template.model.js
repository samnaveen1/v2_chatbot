...
const modelSchema = new mongoose.Schema({
    ...
    templateDisplayName: { type: String, required: true },
    templateFileName: { type: String, required: false },
    printTemplateFileName: { type: String, required: false },
    templateGroup: { type: String, required: true },
    templateSubGroup: { type: String, required: true },
    templateImage: { type: String, required: true },
    maximumBarcodeLen: { type: Number, default: false },
    width: { type: Number, default: false },
...
const model = mongoose.model('print_templates', modelSchema);
const modelSecondary = secondaryDB.model('print_templates', modelSchema);


/// Print template Activity
const collectionActivitySchema = new mongoose.Schema({
    printTemplateId: { type: mongoose.Schema.Types.ObjectId, required: [true, "printTemplateId is required"] },
...
const modelActivity = activitiesDB.model('print_template_activities', collectionActivitySchema);
const modelActivitySecondary = activitiesSecondaryDB.model('print_template_activities', collectionActivitySchema);
...
const printTemplateActivityJoiSchema = Joi.object({
    printTemplateId: Joi.object().required(),
